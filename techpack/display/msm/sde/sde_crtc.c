...
extern void kernfs_wakeup_poll_only(struct kernfs_node *kn);
static void notify_lineptr_work(struct drm_crtc *crtc, void *usr)
{
	struct kernfs_node *kn = (struct kernfs_node *) usr;

	if(!kn)
		return;
	kernfs_wakeup_poll_only(kn);
}
...
static void sde_crtc_lineptr_cb(void *data, ktime_t timestamp)
{
	...
	sde_crtc_event_queue(crtc, notify_lineptr_work, sde_crtc->lineptr_event_sf, false);
