...
/*Added  by gavin.zhao for bug_57705 vxr7200 dp display debug  --begin */
u32 vxr7200_brightness = 0;
		...
		void *display, u32 bl_lvl)
{
	...
	vxr7200_brightness = (u32)bl_temp;
...
/*Added by gavin.zhao for bug_57705 vxr7200 dp display debug  --end */
		...
		struct dsi_panel *panel)
{
	...
	if (rc <= 0) {
        ...
        rc = true; // Modified by PICO_Driver dean.ji for bug 65664 : 增加复位DSI功能
		if (!rc) {
			rc = -EINVAL;
			goto exit;
		}
...
static ssize_t debugfs_backlight_brightness(struct file *file,
				  const char __user *user_buf,
				  size_t user_len,
				  loff_t *ppos)
{
	...
	buf[len] = '\0'; /* terminate the string */
	ptmp = (char*)buf;
	result_1 = strsep(&ptmp, ",");
	if(result_1 != NULL) {
		result_2 = strsep(&ptmp, ",");
		if(result_2 != NULL){
			rc = kstrtoul(result_1, 0, &brightness_1);
			rc = kstrtoul(result_2, 0, &brightness_2);
			printk("set backlight_brightness_1  = %ld\n", brightness_1);
			printk("set backlight_brightness_2  = %ld\n", brightness_2);
			//max bl: jdi(601)
			dsi_panel_neo_update_backlight(panel, (u32)brightness_1, (u32)brightness_2, 72);
		} else {
			printk("set backlight_brightness only one  = %s\n", result_1);
		}
	}
	rc = len;
error:
...
static const struct file_operations backlight_brightness_fops = {
	.open = simple_open,
	.write = debugfs_backlight_brightness,
};
...
static int dsi_display_debugfs_init(struct dsi_display *display)
{
	...
	dump_file = debugfs_create_file("backlight_brightness",
					0644,
					dir,
					display,
					&backlight_brightness_fops);
	if (IS_ERR_OR_NULL(dump_file)) {
		rc = PTR_ERR(dump_file);
		DSI_ERR("[%s] debugfs for backlight_brightness failed, rc=%d\n",
		       display->name, rc);
		goto error_remove_dir;
	}
...
#if 0
		if(!strncmp(display->name,"qcom,mdss_dsi_jdi_493",21)){
			DSI_INFO("jdi 493 panel display\n");
			if(timing->refresh_rate == 120){
				timing->v_back_porch = 7;
				timing->v_front_porch = 14;
				adj_mode->timing.v_back_porch = 7;
				adj_mode->timing.v_front_porch = 14;
			} else if(timing->refresh_rate == 72){
				timing->v_back_porch = 20;
				timing->v_front_porch = 2459;
				adj_mode->timing.v_back_porch = 20;
				adj_mode->timing.v_front_porch = 2459;
			} else if(timing->refresh_rate == 90){
				timing->v_back_porch = 20;
				timing->v_front_porch = 1231;
				adj_mode->timing.v_back_porch = 20;
				adj_mode->timing.v_front_porch = 1231;
			}
		}else if(!strncmp(display->name,"qcom,mdss_dsi_sharp_493",23)){
			DSI_INFO("sharp 493 panel display\n");
			if(timing->refresh_rate == 120){
				timing->v_back_porch = 9;
				timing->v_front_porch = 14;
				adj_mode->timing.v_back_porch = 9;
				adj_mode->timing.v_front_porch = 14;
			} else if(timing->refresh_rate == 72){
				timing->v_back_porch = 20;
				timing->v_front_porch = 2459;
