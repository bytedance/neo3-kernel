...
/*added by webber.wang for Bug 38813:unbalanced ldo 22--start*/
static int dsi_panel_chahei_vreg_get(struct dsi_panel *panel)
{
	int rc = 0;
	int i;
	struct regulator *vreg = NULL;

	for (i = 0; i < panel->chahei_power_info.count; i++) {
		vreg = devm_regulator_get(panel->parent,
					  panel->chahei_power_info.vregs[i].vreg_name);
		rc = PTR_RET(vreg);
		if (rc) {
		...
		panel->chahei_power_info.vregs[i].vreg = vreg;
	}

	return rc;
error_put:
	for (i = i - 1; i >= 0; i--) {
		devm_regulator_put(panel->chahei_power_info.vregs[i].vreg);
		panel->chahei_power_info.vregs[i].vreg = NULL;
	}
	return rc;
}

static int dsi_panel_chahei_vreg_put(struct dsi_panel *panel)
{
	int rc = 0;
	int i;

	for (i = panel->chahei_power_info.count - 1; i >= 0; i--)
		devm_regulator_put(panel->chahei_power_info.vregs[i].vreg);

	return rc;
}
/*added by webber.wang for Bug 38813:unbalanced ldo 22--end*/
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	/*Added by gavin.zhao for sx2130 lcd vio enable begin*/
	if (gpio_is_valid(panel->panel_vio_gpio)) {
		rc = gpio_request(panel->panel_vio_gpio, "panel_vio_gpio");
		if (rc) {
			DSI_WARN("request for panel_vio_gpio failed, rc=%d\n",
				 rc);
			goto error_release_vio_enable;
		}
	}
	/*Added by gavin.zhao for sx2130 lcd vio enable end*/
	/*Added by gavin.zhao for sx2130 lcd blkd enable begin*/
	if (gpio_is_valid(panel->panel_blk_gpio)) {
		rc = gpio_request(panel->panel_blk_gpio, "panel_blk_gpio");
		if (rc) {
			DSI_WARN("request for panel_blk_gpio failed, rc=%d\n",
				 rc);
			goto error_release_vio_enable;
		}
	}
	/*Added by gavin.zhao for sx2130 lcd blk enable end*/
...
static int dsi_panel_gpio_release(struct dsi_panel *panel)
{
	...
	/*Added by gavin.zhao for sx2130 lcd vio enable begin*/
	if (gpio_is_valid(panel->panel_vio_gpio))
		gpio_free(panel->panel_vio_gpio);
	/*Added by gavin.zhao for sx2130 lcd vio enable end*/
	/*Added by gavin.zhao for sx2130 lcd blk enable begin*/
	if (gpio_is_valid(panel->panel_blk_gpio))
		gpio_free(panel->panel_blk_gpio);
	/*Added by gavin.zhao for sx2130 lcd blk enable end*/
...
int dsi_panel_reset(struct dsi_panel *panel)
{
    ...
    /*deleted by webber.wang for Bug 44273: device shut down after sleep-active--start*/
    #if 0
	if (gpio_is_valid(panel->bl_config.en_gpio)) {
    ...
    #endif
    /*deleted by webber.wang for Bug 44273: device shut down after sleep-active--end*/
	if (gpio_is_valid(panel->reset_config.lcd_mode_sel_gpio)) {
		bool out = true;
...
/* Added by gavin.zhao for bug_54373 pwm backlight -- start */
static int blk_set_pwm(struct dsi_panel *panel, int duty, int period)
{
	int rc = 0;
	struct pwm_state pstate;

	pwm_get_state(panel->lcd_pwm, &pstate);
	pstate.period = period;
	pstate.duty_cycle = duty;
	pstate.output_type = PWM_OUTPUT_FIXED;
	pstate.output_pattern = NULL;
	pstate.enabled = duty ? true : false;
	rc = pwm_apply_state(panel->lcd_pwm, &pstate);
	if (rc < 0)
                 pr_err("unable to set pwm\n");
	return rc;
}
/* Added by gavin.zhao for bug_54373 pwm backlight -- end */

u32 vxr7200_panel_off = 0;
int dsi_panel_power_on(struct dsi_panel *panel)
{
	int rc = 0;

	vxr7200_panel_off = 0;
	/*Added by gavin.zhao for sxr2130 lcd bring up vio enable begin */
	if (gpio_is_valid(panel->panel_vio_gpio)) {
		gpio_direction_output(panel->panel_vio_gpio, 1);
	}
	/*Added by gavin.zhao for sxr2130 lcd bring up vio enable end */
...
#if 0
	/* Added by gavin.zhao for bug_54373 pwm backlight -- start */
	blk_set_pwm(panel, 700000, 2000000);
        mdelay(100);
        blk_set_pwm(panel, 790000, 2000000);
        mdelay(130);
        blk_set_pwm(panel, 820000, 2000000);
        mdelay(170);
        blk_set_pwm(panel, 840000, 2000000);
        mdelay(50);
        blk_set_pwm(panel, 850000, 2000000);
        mdelay(50);
        blk_set_pwm(panel, 2000000, 2000000);
        /* Added by gavin.zhao for bug_54373 pwm backlight -- end */
	/*Added by gavin.zhao for sxr2130 lcd bring up blk enable begin */
	if (gpio_is_valid(panel->panel_blk_gpio)) {
		gpio_direction_output(panel->panel_blk_gpio, 1);
	}
	/*Added by gavin.zhao for sxr2130 lcd bring up blk enable end */
#endif
	goto exit;
...
error_disable_vregs:
	(void)dsi_pwr_enable_regulator(&panel->chahei_power_info, false);//added by webber.wang for Bug 38813:unbalanced ldo 22
...
int g_hardware_version_display = -1;
int dsi_panel_power_off(struct dsi_panel *panel)
{
	int rc = 0;

	vxr7200_panel_off = 1;
	/* Added by gavin.zhao for bug_39719 set backlight enable gpio to low when power_off --begin  */
    /*modified by webber.wang for Bug 47070: [falconcv2] capacitor charge use pmic-pwm after ES2--start*/
        if (gpio_is_valid(panel->bl_config.en_gpio))
            gpio_set_value(panel->bl_config.en_gpio, 0);

//	blk_set_pwm(panel, 0, 2000000);
    /*modified by webber.wang for Bug 47070: [falconcv2] capacitor charge use pmic-pwm after ES2--end*/
	/* Added by gavin.zhao for bug_39719 set backlight enable gpio to low when power_off --end  */
    ...
    /*Added by gavin.zhao for bug_43827 power off timing according to spec --start */
	msleep(25);
	/*Added by gavin.zhao for bug_43827 power off timing according to spec --end */
    /*added by webber.wang for bug 47188: backlight not off when charge in power off --start*/
    if(g_hardware_version_display == -1){
       g_hardware_version_display = hw_version_get();
    }
    if(g_hardware_version_display < HARDWARE_SC8201_NUM){
    rc = dsi_pwr_enable_regulator(&panel->chahei_power_info, false);
	if (rc)
	    pr_err("[%s] failed to disable chahei_power_info, rc=%d\n", panel->name, rc);
    }
    /*added by webber.wang for bug 47188: backlight not off when charge in power off --end*/
	...
	/*Added by gavin.zhao for sxr2130 lcd bring up blk enable begin */
	if (gpio_is_valid(panel->panel_blk_gpio)) {
		gpio_direction_output(panel->panel_blk_gpio, 0);
	}
	/*Added by gavin.zhao for sxr2130 lcd bring up blk enable end */
	/*Added by gavin.zhao for sxr2130 lcd bring up vio enable begin */
	if (gpio_is_valid(panel->panel_vio_gpio)) {
		gpio_direction_output(panel->panel_vio_gpio, 0);
	}
	/*Added by gavin.zhao for sxr2130 lcd bring up vio enable end */
...
/*added by webber for task 32828: brightness can be modified--start*/
/*modified by webber.wang for Bug 47070: [falconcv2] capacitor charge use pmic-pwm after ES2--start*/
static int dsi_panel_update_backlight_gpio(struct dsi_panel *panel, u32 bl_lvl)
{
	//unsigned int i=0;
	unsigned int ret = 0;
	static unsigned int bklt_status = 1;
	...
	switch(bl_lvl)
	{
		...
		if(g_hardware_version_display >= HARDWARE_SC8201_NUM){
			ret = dsi_pwr_enable_regulator(&panel->chahei_power_info, false);
			if (ret)
				pr_err("[%s] failed to disable chahei_power_info, ret=%d\n", panel->name, ret);
		}

                if (gpio_is_valid(panel->bl_config.en_gpio)) {
                    pr_debug("%s line%d: set bklt gpio\n",__func__,__LINE__);
                    gpio_direction_output(panel->bl_config.en_gpio, 0);
                    if (ret) {
                        pr_err("%s line%d : unable to set dir for bklt gpio\n",
                        __func__,__LINE__);
                    }
                }
			blk_set_pwm(panel, 0, 2000000);
                	bklt_status = 0;
			/*Added by gavin.zhao for sxr2130 lcd SC8201 enable begin */
			if (gpio_is_valid(panel->panel_blk_gpio)) {
				gpio_direction_output(panel->panel_blk_gpio, 0);
			}
			/*Added by gavin.zhao for sxr2130 lcd SC8201 enable end */
			pr_debug("added by webber to see bklt gpio case 0\n");
			return ret;
		case 1:
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[0]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[0],0);
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[1]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[1],0);
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[2],0);
			pr_debug("added by webber to see bklt gpio case 1\n");
			break;

		case 2:
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[0]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[0],1);
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[1]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[1],0);
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[2],0);
			pr_debug("added by webber to see bklt gpio case 2\n");
			break;
		...
		case 3:
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[0]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[0],0);
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[1]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[1],1);
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[2],0);
			pr_debug("added by webber to see bklt gpio case 3\n");
			break;
		...
		case 4:
			...
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[1]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[1],0);
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[2],1);
			pr_debug("added by webber to see bklt gpio case 4\n");
			break;
		case 5:
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[0]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[0],1);
			...
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[2],1);
			pr_debug("added by webber to see bklt gpio case 5\n");
			break;
		case 6:
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[0]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[0],0);
			...
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[2],1);
			pr_debug("added by webber to see bklt gpio case 6\n");
			break;
		case 7:
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[0]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[0],1);
			...
			if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2]))
				gpio_direction_output(panel->bl_config.bklt_brightness_gpio[2],1);
			pr_debug("added by webber to see bklt gpio case 7\n");
			break;
		default:
			break;
	}
	...
	blk_set_pwm(panel, 700000, 2000000);
        ...
        blk_set_pwm(panel, 2000000, 2000000);
         ...
         if (gpio_is_valid(panel->bl_config.en_gpio)) {
                pr_debug("%s line%d: set bklt gpio\n",__func__,__LINE__);
                gpio_direction_output(panel->bl_config.en_gpio, 1);
                if (ret) {
                    pr_err("%s line%d : unable to set dir for bklt gpio\n",__func__,__LINE__);
                }
         }
	...
	/*modified by webber.wang for Bug 38813:unbalanced ldo 22--start*/
	if(0 == regulator_is_enabled(panel->chahei_power_info.vregs[0].vreg))
	{
		ret = dsi_pwr_enable_regulator(&panel->chahei_power_info, true);
		if (ret) {
			pr_err("[%s] failed to enable panel chahei vregs, rc=%d\n", panel->name, ret);
		}
	}
	/*modified by webber.wang for Bug 38813:unbalanced ldo 22--end*/
	return ret;
}
/*modified by webber.wang for Bug 47070: [falconcv2] capacitor charge use pmic-pwm after ES2--end*/
/*added by webber for task 32828: brightness can be modified--end*/
...
int dsi_panel_neo_update_backlight(struct dsi_panel *panel,
			u32 bl_lvl_1, u32 bl_lvl_2, u32 fps)
{
	int rc = 0;
	...
	if (!panel || !panel->cur_mode || (bl_lvl_1 > 0xffff) || (bl_lvl_2 > 0xffff))
		return -EINVAL;
	...
	//jdi duty:100(0x171); sharp duty:78(0x118)
	if(!strncmp(panel->name,"jdi 493 120 new video",21)){
		printk("dsi_panel_neo_update_backlight enter jdi panel\n");
		bl_lvl_1 = (u32)((bl_lvl_1 * vtotal * 100) / 1000000);
		bl_lvl_2 = (u32)((bl_lvl_2 * vtotal * 100) / 1000000);
	} else if(!strncmp(panel->name,"sharp 493 120 new video",23)){
		printk("dsi_panel_neo_update_backlight enter sharp panel\n");
		bl_lvl_1 = (u32)((bl_lvl_1 * vtotal * 80) / 1000000);
		bl_lvl_2 = (u32)((bl_lvl_2 * vtotal * 80) / 1000000);
	} else {
		printk("enter other panel\n");
		goto error;
	}

	if(fps == 120){
		printk("dsi_panel_neo_update_backlight 120fps\n");
		payload[0] = 0x0C; //from panel reg
		payload[1] = 0xF4;
		payload[8] = 0x05;
		payload[9] = 0xB4;
	} else if(fps == 90){
		printk("dsi_panel_neo_update_backlight 90fps\n");
		payload[0] = 0x11; //from panel reg
		payload[1] = 0xA1;
		payload[8] = 0x05;
		payload[9] = 0x8E;
	} else { // fps == 72
		printk("dsi_panel_neo_update_backlight 72fps\n");
		payload[0] = 0x11; //from panel reg
		payload[1] = 0xA1;
		payload[8] = 0x00;
		payload[9] = 0xBE;
	...
	payload[2] = bl_lvl_1 >> 8;
	payload[3] = bl_lvl_1 & 0xff;

	payload[10] = bl_lvl_2 >> 8;
	payload[11] = bl_lvl_2 & 0xff;

	rc = mipi_dsi_dcs_write(dsi, reg, payload, sizeof(payload));
	printk("dsi_panel_neo_update_backlight payload[2]=0x%x, payload[3]=0x%x\n",payload[2],payload[3] );
	printk("dsi_panel_neo_update_backlight payload[10]=0x%x, payload[11]=0x%x\n",payload[10],payload[11] );
	if (rc) {
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_DCS:
		...
		break;
	/*added by webber for task 32828: brightness can be modified--start*/
	case DSI_BACKLIGHT_GPIO:
		dsi_panel_update_backlight_gpio(panel,bl_lvl);
		break;
	/*added by webber for task 32828: brightness can be modified--end*/
...
static u32 dsi_panel_get_brightness(struct dsi_backlight_config *bl)
{
	...
	case DSI_BACKLIGHT_WLED:
		...
		break;
	case DSI_BACKLIGHT_DCS:
	case DSI_BACKLIGHT_GPIO:/*added by webber for task 32828: brightness can be modified*/
	case DSI_BACKLIGHT_EXTERNAL:
	case DSI_BACKLIGHT_PWM:
	default:
		/*
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-post-72-panel-on-command",
	"qcom,mdss-dsi-post-90-panel-on-command",
	"qcom,mdss-dsi-post-120-panel-on-command",
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-post-72-command-state",
	"qcom,mdss-dsi-post-90-command-state",
	"qcom,mdss-dsi-post-120-command-state",
...
/*added by webber.wang for Bug 38813:unbalanced ldo 22--start*/
static int dsi_panel_chahei_parse_power_cfg(struct device *parent,
				     struct dsi_panel *panel,
				     struct device_node *of_node)
{
	int rc = 0;
	...
	rc = dsi_pwr_of_get_vreg_data(&panel->utils,
					  &panel->chahei_power_info,
					  "qcom,panel-chahei-supply-entries");
	if(panel->chahei_power_info.vregs == NULL)
		pr_err("webber dsi_panel_chahei_parse_power_cfg null\n");
	else
		pr_err("webber dsi_panel_chahei_parse_power_cfg ok\n");
	if (rc) {
		pr_err("[%s] failed to parse chahei vregs\n", panel->name);
		goto error;
	}

        /* Added by gavin.zhao for bug_54373 pwm backlight -- start */
	panel->lcd_pwm = devm_of_pwm_get(panel->parent, panel->panel_of_node, NULL);
        if (IS_ERR(panel->lcd_pwm)) {
                 pr_err("sxr2130 unable to request lcd PWM\n");
        }
        pwm_apply_args(panel->lcd_pwm);//added by webber.wang for Bug 47070: [falconcv2] capacitor charge use pmic-pwm after ES2
        /* Added by gavin.zhao for bug_54373 pwm backlight -- end */
error:
	return rc;
}
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	if (!gpio_is_valid(panel->panel_test_gpio))
		DSI_DEBUG("%s:%d panel test gpio not specified\n", __func__,
			 __LINE__);
	/*Added by gavin.zhao for sxr2130 lcd bring up vio enable begin */
	panel->panel_vio_gpio = utils->get_named_gpio(utils->data,
					"qcom,display-vio-en-gpio",
					0);
	if (!gpio_is_valid(panel->panel_vio_gpio))
		DSI_DEBUG("%s:%d panel vio enable gpio not specified\n", __func__,
			 __LINE__);
	/*Added by gavin.zhao for sxr2130 lcd bring up vio enable end */
	/*Added by gavin.zhao for sxr2130 lcd bring up blk  enable begin */
	panel->panel_blk_gpio = utils->get_named_gpio(utils->data,
					"qcom,platform-bklight-en-gpio_2",
					0);
	if (!gpio_is_valid(panel->panel_blk_gpio))
		DSI_DEBUG("%s:%d panel blk enable gpio not specified\n", __func__,
			 __LINE__);
	/*Added by gavin.zhao for sxr2130 lcd bring up blk enable end */
error:
	return rc;
...
/*added by webber for task 32828: brightness can be modified--start*/
	} else if (!strcmp(bl_type, "bl_ctrl_gpio")) {
		panel->bl_config.type = DSI_BACKLIGHT_GPIO;
		panel->bl_config.bklt_brightness_gpio[0] = utils->get_named_gpio(utils->data,
				"qcom,mdss-dsi-bklight-gpio1", 0);
		if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[0])){
			pr_info("%s: qcom,mdss-dsi-bklight-gpio1 specified\n", __func__);

		rc = gpio_request(panel->bl_config.bklt_brightness_gpio[0],"bklight-gpio1");
		if (rc)
			pr_err("request mdss-dsi-bklight-gpio1 failed, rc=%d\n",rc);
		}

		panel->bl_config.bklt_brightness_gpio[1] = utils->get_named_gpio(utils->data,
				"qcom,mdss-dsi-bklight-gpio2", 0);
		if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[1])){
			pr_info("%s: qcom,mdss-dsi-bklight-gpio2  specified\n", __func__);

		rc = gpio_request(panel->bl_config.bklt_brightness_gpio[1],"bklight-gpio2");
		if (rc)
			pr_err("request mdss-dsi-bklight-gpio2 failed, rc=%d\n",rc);
		}

		panel->bl_config.bklt_brightness_gpio[2] = utils->get_named_gpio(utils->data,
				"qcom,mdss-dsi-bklight-gpio3", 0);
		if (gpio_is_valid(panel->bl_config.bklt_brightness_gpio[2])){
			pr_err("%s: qcom,mdss-dsi-bklight-gpio3  specified\n", __func__);

		rc = gpio_request(panel->bl_config.bklt_brightness_gpio[2],"bklight-gpio3");
		if (rc)
...
/*Modify  by PICO_Driver gavin.zhao for Bug_57705 vxr7200 debug  --- start */
struct dsi_panel *vxr7200_panel;
/*Modify  by PICO_Driver gavin.zhao for Bug_57705 vxr7200 debug  --- end */
				...
				int topology_override)
{
	...
	/*added by webber.wang for Bug 38813:unbalanced ldo 22--start*/
	rc = dsi_panel_chahei_parse_power_cfg(parent, panel, of_node);
	if (rc)
			pr_err("failed to parse panel chahei power config, rc=%d\n", rc);
	/*added by webber.wang for Bug 38813:unbalanced ldo 22--end*/
		       ...
		       struct mipi_dsi_host *host)
{
	...
	/*added by webber.wang for Bug 38813:unbalanced ldo 22--start*/
	rc = dsi_panel_chahei_vreg_get(panel);
	if (rc) {
...
int dsi_panel_drv_deinit(struct dsi_panel *panel)
{
	...
	/*added by webber.wang for Bug 38813:unbalanced ldo 22--start*/
	rc = dsi_panel_chahei_vreg_put(panel);
	if (rc)
...
int dsi_panel_post_enable_vxr7200(struct dsi_panel *panel)
{
	int rc = 0;
    ...
    pr_err("vxr7200 fps:%d\n",panel->cur_mode->timing.refresh_rate);
	mutex_lock(&panel->panel_lock);
    if(panel->cur_mode->timing.refresh_rate == 72) 	rc = dsi_panel_neo_update_backlight(panel,1000,1000,72);
    if(panel->cur_mode->timing.refresh_rate == 90) 	rc = dsi_panel_neo_update_backlight(panel,1000,1000,90);
    if(panel->cur_mode->timing.refresh_rate == 120)	rc = dsi_panel_neo_update_backlight(panel,1000,1000,120);
	if (rc) {
...
/* Added by Gavin.zhao for task_2155 72/90 switch --begin */
int dsi_panel_post_enable_72(struct dsi_panel *panel)
{
	int rc = 0;
	...
	rc = dsi_panel_neo_update_backlight(panel,1000,1000,72);
//	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_72);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_ON cmds, rc=%d\n",
		       panel->name, rc);
		goto error;
...
int dsi_panel_post_enable_90(struct dsi_panel *panel)
{
	int rc = 0;
	...
	rc = dsi_panel_neo_update_backlight(panel,1000,1000,90);
	//rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_90);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_ON cmds, rc=%d\n",
		       panel->name, rc);
		goto error;
...
int dsi_panel_post_enable_120(struct dsi_panel *panel)
{
	int rc = 0;
	...
	rc = dsi_panel_neo_update_backlight(panel,1000,1000,120);
	//rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_120);
	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_ON cmds, rc=%d\n",
		       panel->name, rc);
		goto error;
...
/*Added by gavin.zhao hunter.zhao for bug54373 sx2130 lcd blkd disable delay 40ms start--*/
	msleep(40);
/*Added by gavin.zhao hunter.zhao for bug54373 sx2130 lcd blkd disable delay 40ms end--*/
	mutex_lock(&panel->panel_lock);
