...
static void dsi_bridge_enable(struct drm_bridge *bridge)
{
	...
	if(cur->timing.refresh_rate == 72) {
		rc = dsi_panel_post_enable_72(display->panel);
	}
	if (cur->timing.refresh_rate == 90) {
		rc = dsi_panel_post_enable_90(display->panel);
	}
        /* Added by Gavin.zhao for task_2155 72/90 switch --end */
        /* Added by Gavin.zhao for task_2155 72/90 switch --begin */
        if (cur->timing.refresh_rate == 120) {
              rc = dsi_panel_post_enable_120(display->panel);
        }
        /* Added by Gavin.zhao for task_2155 72/90 switch --end */
