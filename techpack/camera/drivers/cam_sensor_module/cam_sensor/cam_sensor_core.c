...
int cam_sensor_is_eyetracking(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if((io_master_info->cci_client->sid == 0x75)||(io_master_info->cci_client->sid == 0x76))
	{
		return 1;
	}
	...
	struct i2c_settings_list *i2c_list)
{
	...
	if(cam_sensor_is_eyetracking(s_ctrl) == 1)
	{
		/*Modify by PICO_Driver sophia.wang for bug57941: add eyetracking led always on mode for cit test start*/
		if((eyetracking_led_always_on == 1)&&(i2c_list->i2c_settings.size != 14))//for led always on ,some hardware devices use master strobe and others use slave strobe
		{
			for(j = 0; j < i2c_list->i2c_settings.size; j++)
			{
				if(i2c_list->i2c_settings.reg_setting[j].reg_addr == 0x3027)
				{
					i2c_list->i2c_settings.reg_setting[j].reg_data = 0x08;
				}
				else if(i2c_list->i2c_settings.reg_setting[j].reg_addr == 0x3009)
				{
					i2c_list->i2c_settings.reg_setting[j].reg_data = 0x08;
				}
		...
		/*Modify by PICO_Driver sophia.wang for bug57941: add eyetracking led always on mode for cit test end*/
		/*Modify by PICO_Driver sophia.wang for bug63400:let the eyetracking camera master and slave use the same exposure parameter start*/
		if((i2c_list->i2c_settings.size == 14)&&(io_master_info->cci_client->sid == 0x76))
		{
			io_master_info->cci_client->sid = 0xC0 >> 1;//use the public slave addr to write exposure for two eyetracking camera
		}
		else if((i2c_list->i2c_settings.size == 14)&&(io_master_info->cci_client->sid == 0x75))
		{
			return rc;
		}
		/*Modify by PICO_Driver sophia.wang for bug63400:let the eyetracking camera master and slave use the same exposure parameter start*/
		CAM_INFO(CAM_SENSOR,"sophia eyetracking size %d,addr 0x%x reg_data 0x%x sid %d",i2c_list->i2c_settings.size,i2c_list->i2c_settings.reg_setting[0].reg_addr,i2c_list->i2c_settings.reg_setting[0].reg_data,io_master_info->cci_client->sid);
}
else
{
	#ifdef NEO25_FOUR_6DOF_CAMERAS_SUPPORT
...
static ssize_t sixdof_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
		return sprintf(buf, "%d\n", (0x0F==(camera_exist&0x0F)));
}
static DEVICE_ATTR( sixdof, 0664, sixdof_show, NULL);

/*Modify by PICO_Driver sophia.wang for bug56388 : add a node for eyetracking led debug start*/
static ssize_t eyetracking_led_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
		return sprintf(buf, "%d\n", eyetracking_led_always_on);
}
static ssize_t eyetracking_led_store(struct device *dev,
			struct device_attribute *attr, const char *buf, size_t count)
{
		...
		CAM_DBG(CAM_SENSOR, "set eyetracking_led_always_on:%d",value);
		eyetracking_led_always_on = value;
		return count;
}
static DEVICE_ATTR( eyetracking_led, 0664, eyetracking_led_show, eyetracking_led_store);
/*Modify by PICO_Driver sophia.wang for bug56388 : add a node for eyetracking led debug end*/

static ssize_t eyetracking_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
		return sprintf(buf, "%d\n", (0x30==(camera_exist&0x30)));
}
static DEVICE_ATTR( eyetracking, 0664, eyetracking_show, NULL);

static struct attribute *sixdof_attributes[] = {
	&dev_attr_sixdof.attr,
	NULL
};
static struct attribute_group sixdof_attribute_group = {
	.attrs = sixdof_attributes
};

static struct attribute *eyetracking_attributes[] = {
	&dev_attr_eyetracking.attr,
	&dev_attr_eyetracking_led.attr,/*Modify by PICO_Driver sophia.wang for bug56388 : add a node for eyetracking led debug*/
	NULL
};
static struct attribute_group eyetracking_attribute_group = {
	.attrs = eyetracking_attributes
};
...
void cam_sensor_exist_status(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	/*6dof*/
	if((s_ctrl->soc_info.index == 4)&&(slave_info->sensor_id == 0x7750)&&(0 == (camera_exist&0x1)))
	{
		camera_exist = camera_exist|0x1;
		if(0xF == (camera_exist&0xF))
		{
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group) < 0) {
						CAM_ERR(CAM_SENSOR, "%s: create sixdof_attribute_group system file node failure!\n", __func__);
					}
		}
	}
	else if((s_ctrl->soc_info.index == 5)&&(slave_info->sensor_id == 0x7750)&&(0 == (camera_exist&0x2)))
	{
		camera_exist = camera_exist|0x2;
		if(0xF == (camera_exist&0xF))
		{
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group) < 0) {
	...
	else if((s_ctrl->soc_info.index == 2)&&(slave_info->sensor_id == 0x7750)&&(0 == (camera_exist&0x4)))
	{
		camera_exist = camera_exist|0x4;
		if(0xF == (camera_exist&0xF))
		{
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group) < 0) {
	...
	else if((s_ctrl->soc_info.index == 3)&&(slave_info->sensor_id == 0x7750)&&(0 == (camera_exist&0x8)))
	{
		camera_exist = camera_exist|0x8;
		if(0xF == (camera_exist&0xF))
		{
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &sixdof_attribute_group) < 0) {
	...
	/*eyetracking*/
	if((s_ctrl->soc_info.index == 0)&&(slave_info->sensor_id == 0x6710)&&(0 == (camera_exist&0x10)))
	{
		camera_exist = camera_exist|0x10;
		if(0x30 == (camera_exist&0x30))
		{
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group) < 0) {
						CAM_ERR(CAM_SENSOR, "%s: create eyetracking_attribute_group system file node failure!\n", __func__);
					}
		}
	}
	else if((s_ctrl->soc_info.index == 1)&&(slave_info->sensor_id == 0x6710)&&(0 == (camera_exist&0x20)))
	{
		camera_exist = camera_exist|0x20;
		if(0x30 == (camera_exist&0x30))
		{
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group) < 0) {
	...
	if((s_ctrl->soc_info.index == 0)&&(slave_info->sensor_id == 0x0476)&&(0 == (camera_exist&0x10)))
	{   CAM_INFO(CAM_SENSOR, "IMX476 0x10 0\n");
		camera_exist = camera_exist|0x10;
		if(0x30 == (camera_exist&0x30))
		{   CAM_INFO(CAM_SENSOR, "IMX476 0x10 1\n");
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group) < 0) {
						CAM_ERR(CAM_SENSOR, "%s: create eyetracking_attribute_group system file node failure!\n", __func__);
					}
	...
	else if((s_ctrl->soc_info.index == 1)&&(slave_info->sensor_id == 0x0476)&&(0 == (camera_exist&0x20)))
	{   CAM_INFO(CAM_SENSOR, "IMX476 0x10 00\n");
		camera_exist = camera_exist|0x20;
		if(0x30 == (camera_exist&0x30))
		{   CAM_INFO(CAM_SENSOR, "IMX476 0x10 11\n");
			sysfs_remove_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group);
			if (sysfs_create_group(&s_ctrl->pdev->dev.kobj, &eyetracking_attribute_group) < 0) {
						CAM_ERR(CAM_SENSOR, "%s: create eyetracking_attribute_group system file node failure!\n", __func__);
					}
