...
static int32_t cam_sensor_driver_get_dt_data(struct cam_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	int i = 0;
	int hw_version = 0;/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not start*/
...
/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not start*/
	...
	{
		rc = cam_soc_util_get_dt_properties_pico_r3(soc_info);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "Failed to read DT properties rc %d", rc);
			goto FREE_SENSOR_DATA;
		}

		rc =  cam_sensor_util_init_gpio_pin_tbl_pico_r3(soc_info,
				&sensordata->power_info.gpio_num_info);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "Failed to read gpios %d", rc);
			goto FREE_SENSOR_DATA;
