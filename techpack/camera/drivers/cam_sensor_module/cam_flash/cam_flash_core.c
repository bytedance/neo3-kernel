...
#include <linux/gpio.h>/*Modify by PICO_Driver sophia.wang for bug55005 : eyetracking camera in sxr2130 flash_3*/
...
/*Modify by PICO_Driver sophia.wang for bug55005 : eyetracking camera in sxr2130 flash_3 start*/
static void cam_flash_set_gpios(struct cam_flash_ctrl *flash_ctrl,
	bool enable)
{
	int gpio_strobe = 1124;/*Modify by PICO_Driver sophia.wang for bug55317 :  eyetracking camera in sxr2130 flash_4 */

	pr_err(" wq++++++++++++++++++++ cam_flash_set_gpios enable = %d\n",enable);

	if(((enable)&&(gpio_get_value(gpio_strobe) == 1))
		||((!enable)&&(gpio_get_value(gpio_strobe) == 0)))
	{
		pr_err(" wq++++++++++++++++++++  et_led state no change,so go back");
		return;
	}

	gpio_set_value(gpio_strobe, (enable ? 1 : 0));

	pr_err(" wq++++++++++++++++++++  set et_led gpio24 = %d",enable);


}
/*Modify by PICO_Driver sophia.wang for bug55005 : eyetracking camera in sxr2130 flash_3 end*/
	...
	struct cam_flash_frame_setting *flash_data, enum camera_flash_opcode op)
{
	...
	if (op == CAMERA_SENSOR_FLASH_OP_FIRELOW) {
		...
		cam_flash_set_gpios(flash_ctrl, true);/*Modify by PICO_Driver sophia.wang for bug55005 : eyetracking camera in sxr2130 flash_3*/
...
int cam_flash_off(struct cam_flash_ctrl *flash_ctrl)
{
	...
	cam_flash_set_gpios(flash_ctrl, false);/*Modify by PICO_Driver sophia.wang for bug55005 : eyetracking camera in sxr2130 flash_3*/
