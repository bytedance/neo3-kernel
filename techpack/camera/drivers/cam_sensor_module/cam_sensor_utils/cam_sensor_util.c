		...
		case SENSOR_VDIG:
			for (j = 0; j < num_vreg; j++) {
				if ((!strcmp(soc_info->rgltr_name[j],	"cam_vdig"))||
					(!strcmp(soc_info->rgltr_name[j],"r3-cam_vdig")))/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not*/
					{
		...
		case SENSOR_VIO:
			for (j = 0; j < num_vreg; j++) {
				...
				if ((!strcmp(soc_info->rgltr_name[j],	"cam_vio")) ||
					(!strcmp(soc_info->rgltr_name[j],"r3-cam_vio")))/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not*/
					{
					CAM_DBG(CAM_SENSOR,
						"i: %d j: %d cam_vio", i, j);
					power_setting[i].seq_val = j;
		...
		case SENSOR_VANA:
			for (j = 0; j < num_vreg; j++) {
				...
				if ((!strcmp(soc_info->rgltr_name[j],"cam_vana"))||
					(!strcmp(soc_info->rgltr_name[j],"r3-cam_vana")))/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not*/
					{
					CAM_DBG(CAM_SENSOR,
...
/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not start*/
int cam_sensor_util_init_gpio_pin_tbl_pico_r3(
	struct cam_hw_soc_info *soc_info,
	struct msm_camera_gpio_num_info **pgpio_num_info)
{
	...
	*pgpio_num_info = kzalloc(sizeof(struct msm_camera_gpio_num_info),
	...
	rc = of_property_read_u32(of_node, "r3-gpio-vana", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read r3-gpio-vana failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-vana invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-vana %d",
			gpio_num_info->gpio_num[SENSOR_VANA]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-vio", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read gpio-vio failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-vio invalid %d", val);
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-vio %d",
			gpio_num_info->gpio_num[SENSOR_VIO]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-vaf", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read r3-gpio-vaf failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-vaf invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-vaf %d",
			gpio_num_info->gpio_num[SENSOR_VAF]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-vdig", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read r3-gpio-vdig failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-vdig invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-vdig %d",
				gpio_num_info->gpio_num[SENSOR_VDIG]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-reset", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read r3-gpio-reset failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-reset invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-reset %d",
			gpio_num_info->gpio_num[SENSOR_RESET]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-standby", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
			...
			CAM_ERR(CAM_SENSOR, "r3-gpio-standby invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-standby %d",
			gpio_num_info->gpio_num[SENSOR_STANDBY]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-af-pwdm", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"read r3-gpio-af-pwdm failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-af-pwdm invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-af-pwdm %d",
			gpio_num_info->gpio_num[SENSOR_VAF_PWDM]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-custom1", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"read r3-gpio-custom1 failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-custom1 invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-custom1 %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO1]);
	}

	rc = of_property_read_u32(of_node, "r3-gpio-custom2", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"read r3-gpio-custom2 failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "r3-gpio-custom2 invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR, "r3-gpio-custom2 %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO2]);
	} else {
		rc = 0;
	}
...
/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not end*/
	...
	struct cam_hw_soc_info *soc_info, int32_t index)
{
	...
	for (j = 0; j < num_vreg; j++) {
		if( (!strcmp(soc_info->rgltr_name[j], "cam_clk")) ||
			(!strcmp(soc_info->rgltr_name[j], "r3-cam_clk")))/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not*/
			{
			ps = NULL;
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CUSTOM_REG2:
			...
			msleep(10);/*Modify by PICO_Driver sophia.wang for bug55100 : adjust the power sequence for camera*/
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	CAM_INFO(CAM_SENSOR, "soc_info.index = %d", soc_info->index);
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_CUSTOM_REG2:
			...
			CAM_INFO(CAM_SENSOR, "SENSOR_VANA soc_info.index = %d", soc_info->index);
			if((soc_info.index == 2)|| (soc_info.index == 3)||(soc_info.index == 5))
			{
				return 0;
			}
		#endif
			...
			ps = msm_camera_get_power_settings(
				ctrl, pd->seq_type,
				pd->seq_val);
			/*Modify by PICO_Driver sophia.wang for bug55100 : adjust the power sequence for camera start*/
			...
			msleep(10);
			/*Modify by PICO_Driver sophia.wang for bug55100 : adjust the power sequence for camera end*/
