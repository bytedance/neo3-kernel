...
#include <linux/hardware_version.h>/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not*/
...
static int cam_res_mgr_parse_dt_pico_r3(struct device *dev)
{
	int rc = 0;
	...
	rc = of_property_read_u32_array(of_node, "r3-shared-gpios",
...
{
	int rc = 0;
	int hw_version = 0;/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not*/
...
/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not start*/
	#ifdef CAM_RM_PM8009_CONFIG
	rc = cam_res_mgr_parse_dt(&pdev->dev);
	#else
	hw_version = hw_version_get();
	switch(hw_version){
		case 2:
		case 3:
			rc = cam_res_mgr_parse_dt_pico_r3(&pdev->dev);
			break;
		case 4:
		case 5:
			rc = cam_res_mgr_parse_dt(&pdev->dev);
