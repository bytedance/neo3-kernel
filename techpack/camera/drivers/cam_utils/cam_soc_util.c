...
static int cam_soc_util_get_dt_gpio_req_tbl_pico_r3(struct device_node *of_node,
	...
	uint16_t gpio_array_size)
{
	...
	if (!of_get_property(of_node, "r3-gpio-req-tbl-num", &count))
		return 0;

	count /= sizeof(uint32_t);
	if (!count) {
		CAM_ERR(CAM_UTIL, "r3-gpio-req-tbl-num 0");
		return 0;
	}
	...
	rc = of_property_read_u32_array(of_node, "r3-gpio-req-tbl-num",
		val_array, count);
	if (rc) {
		CAM_ERR(CAM_UTIL, "failed in reading r3-gpio-req-tbl-num, rc = %d",
			rc);
	...
	rc = of_property_read_u32_array(of_node, "r3-gpio-req-tbl-flags",
		val_array, count);
	if (rc) {
		CAM_ERR(CAM_UTIL, "Failed in r3-gpio-req-tbl-flags, rc %d", rc);
		goto free_gpio_req_tbl;
	}
	...
	if (!of_get_property(of_node, "r3-gpio-req-tbl-delay", &count)) {
		CAM_DBG(CAM_UTIL, "no r3-gpio-req-tbl-delay");
		kfree(val_array);
		return rc;
	}
	...
	count /= sizeof(uint32_t);
	if (!count) {
		CAM_ERR(CAM_UTIL, "Invalid gpio count for r3-gpio-req-tbl-delay");
		kfree(val_array);
		return rc;
	}
	...
	if (count != gconf->cam_gpio_req_tbl_size) {
		CAM_ERR(CAM_UTIL,
			"Invalid number of r3-gpio-req-tbl-delay entries: %d",
			count);
		goto free_val_array;
	}
	...
	rc = of_property_read_u32_array(of_node, "r3-gpio-req-tbl-delay",
		val_array, count);
	if (rc) {
		CAM_ERR(CAM_UTIL, "Failed to read r3-gpio-req-tbl-delay entry");
	...
	for (i = 0; i < count; i++) {
		...
		CAM_DBG(CAM_UTIL, "r3_gpio_delay_tbl[%d] = %ld", i,
...
/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not start*/
static int cam_soc_util_get_gpio_info_pico_r3(struct cam_hw_soc_info *soc_info)
{
	int32_t rc = 0, i = 0;
	...
	gpio_array_size = of_gpio_named_count(of_node,"r3-gpios");
	...
	for (i = 0; i < gpio_array_size; i++) {
		gpio_array[i] = of_get_named_gpio(of_node,"r3-gpios", i);
	...
	rc = cam_soc_util_get_dt_gpio_req_tbl_pico_r3(of_node, gconf, gpio_array,
		gpio_array_size);
	if (rc) {
...
/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not start*/
static int cam_soc_util_get_dt_regulator_info_pico_r3
	(struct cam_hw_soc_info *soc_info)
{
	...
	count = of_property_count_strings(of_node, "r3-regulator-names");
	...
	for (i = 0; i < soc_info->num_rgltr; i++) {
		rc = of_property_read_string_index(of_node,
			"r3-regulator-names", i, &soc_info->rgltr_name[i]);
	...
	if (!of_property_read_bool(of_node, "r3-rgltr-cntrl-support")) {
	...
	rc = of_property_read_u32_array(of_node, "r3-rgltr-min-voltage",
	...
	rc = of_property_read_u32_array(of_node, "r3-rgltr-max-voltage",
	...
	rc = of_property_read_u32_array(of_node, "r3-rgltr-load-current",
...
/*Add by PICO_Driver sophia.wang for bug64010: do the compatible for having pm8009 or not start*/
int cam_soc_util_get_dt_properties_pico_r3(struct cam_hw_soc_info *soc_info)
{
	struct device_node *of_node = NULL;
	...
	rc = cam_soc_util_get_dt_regulator_info_pico_r3(soc_info);
	if (rc)
		return rc;
	...
	rc = cam_soc_util_get_gpio_info_pico_r3(soc_info);
	if (rc)
		return rc;
