...
struct rb_root avc_clean_caches = RB_ROOT;
DEFINE_MUTEX(avc_clean_mutex);
int avc_clean_cache_hit_count;
#endif
//@}
...
//Smt:[FEAT_LAB_TAG, FEAT_AVC_CLEAN] {@
#if FEAT_LAB_TAG && FEAT_AVC_CLEAN
extern bool isEnableAvcCleanSwitch;
extern int sampling_num_max;
int sampling_count;
struct audit_buffer {
...
static int check_avc_clean_cache(struct common_audit_data *ad) {
    u32 checksum = 0;
    struct avc_clean_cache *cur_node;
    struct avc_clean_cache *new_node;
    struct rb_node *parent = NULL;
    struct rb_node **p = &(avc_clean_caches.rb_node);
    checksum+=ad->selinux_audit_data->ssid;
    checksum+=ad->selinux_audit_data->tsid;
    checksum+=ad->selinux_audit_data->requested;
    checksum+=ad->selinux_audit_data->audited;
    checksum+=ad->selinux_audit_data->tclass;
    mutex_lock(&avc_clean_mutex);
    while (*p) {
        parent = *p;
        cur_node = rb_entry(parent, struct avc_clean_cache, cache_node);
        if (checksum < cur_node->checksum) {
            p = &(*p)->rb_left;
        } else if (checksum > cur_node->checksum) {
            p = &(*p)->rb_right;
        } else {
            avc_clean_cache_hit_count++;
            mutex_unlock(&avc_clean_mutex);
            return 0;
        }
    }
    new_node = kzalloc(sizeof(struct avc_clean_cache), GFP_KERNEL);
    new_node->checksum = checksum;
    rb_link_node(&new_node->cache_node, parent, p);
    rb_insert_color(&new_node->cache_node, &avc_clean_caches);
    mutex_unlock(&avc_clean_mutex);
    return 1;
}
#endif
...
//Smt:[FEAT_LAB_TAG, FEAT_AVC_CLEAN] {@
#if FEAT_LAB_TAG && FEAT_AVC_CLEAN
	if (ad->selinux_audit_data->denied && isEnableAvcCleanSwitch
	    && ++sampling_count == sampling_num_max) {
		struct siginfo info;
		struct nlmsghdr *nlh = nlmsg_hdr(ab->skb);
		char *data = nlmsg_data(nlh);
	    sampling_count = 0;
	    if(!check_avc_clean_cache(ad)) {
	        return;
	    }
		pr_err_ratelimited("avc_clean:%s\n", data);
		clear_siginfo(&info);
		info.si_signo = 35;
		info.si_value.sival_int = 2;
		send_sig_info(35, &info, current);
	}
#endif
//@}
