				...
				union power_supply_propval *val)
{
	...
	/* Added by gavin.zhao for bug_63195 BSM mode recharge ---begin */
	if((battery_soc < 98) && (battery_soc > 0) && (error_recharge == 1)){
		...
		error_recharge = 0;
		printk("smblib_get_prop_batt_health battery_soc = %d \n", battery_soc);
	}
	/* Added by gavin.zhao for bug_63195 BSM mode recharge -- end */
					...
					int usb_current)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		...
		/*Modify  by gavin.zhao for Bug_61188 Fix float charger  --- start */
		if (usb_current < SDP_CURRENT_UA)
			usb_current = SDP_CURRENT_UA;
		/*Modify  by gavin.zhao for Bug_61188 Fix float charger --- end */
				...
				int *total_current_ua)
{
	...
	{
	  *total_current_ua = chg->chg_param.hvdcp3_max_icl_ua;
	  return 0;
	}
...
/* Added by gavin.zhao for bug_63195 BSM mode recharge ---begin */
irqreturn_t error_default_irq_handler(int irq, void *data)
{
	struct smb_irq_data *irq_data = data;
	struct smb_charger *chg = irq_data->parent_data;
	u8 stat;
	...
	printk("enter error_default_irq_handler \n");
	rc = smblib_read(chg, BATTERY_CHARGER_STATUS_2_REG, &stat);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read BATTERY_CHARGER_STATUS_2_REG: rc=%d\n", rc);
		return IRQ_HANDLED;
	}

	if(stat & CHARGER_ERROR_STATUS_EXPIRE_BIT){
		printk("enter error_default_irq_handler enable charge \n");
		error_recharge = 1;
	}
	return IRQ_HANDLED;
}
	...
	if (rising) {
		...
		if (qc_charger) {
			...
			vote(chg->hdc_irq_disable_votable,
	                ...
	                {
	                  vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,chg->chg_param.hvdcp3_max_icl_ua);
                        }
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	/* Added by gavin.zhao for bug_65233 Fix usb slow insertion --start */
	if(apsd_result->bit & (QC_3P0_BIT | QC_2P0_BIT | DCP_CHARGER_BIT)){
		vote(chg->usb_icl_votable, USB_PSY_VOTER, false, 0);
		smblib_run_aicl(chg, RERUN_AICL);
	}
	smblib_dbg(chg, PR_INTERRUPT, "apsd_result bit =%x\n", apsd_result->bit);
	if((apsd_result->bit & (FLOAT_CHARGER_BIT | SDP_CHARGER_BIT | OCP_CHARGER_BIT)) && (float_test_num < 30)){
		smblib_rerun_apsd_if_required(chg);
		float_test_num++;
	}
	/* Added by gavin.zhao for bug_65233 Fix usb slow insertion --end */

	#if 0
	/* Added by PICO_Driver gavin.zhao for bug31271：force 9v output --start-- */
        if((apsd_result->bit & (QC_3P0_BIT | QC_2P0_BIT))
                // && (get_client_vote_locked(chg->qnovo_disable_votable, DEFAULT_VOTER))
                // && (!get_effective_result(chg->hvdcp_disable_votable_indirect))
                 && (!chg->qc3_force_9v) ){ //not yet force 9v
                chg->qc3_force_9v = true;
                ...
                /* Modify by PICO_Driver gavin.zhao for bug44005：disable 9v output --start-- */
                        msleep(300);
                        smblib_masked_write(chg, CMD_HVDCP_2_REG, FORCE_9V_BIT, FORCE_9V_BIT);

                }
                /* Modify by PICO_Driver gavin.zhao for bug47829：usbin_uv disable 9v output --end-- */
                /* Modify by PICO_Driver gavin.zhao for bug44005：disable 9v output --end-- */
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
	...
	/* Modify by gavin.zhao for bug_65233 Fix usb slow insertion --start */
...
#define    DCIN_VOL_THRES       7000000

static int pvr_read_mid_voltage_chan(struct smb_charger *chg,
					union power_supply_propval *val)
{
	int rc;
...
static void dcin_aicl(struct smb_charger *chg)
{
	...
	/* Added by gavin.zhao for task_58666 DC_IN charging debug --begin */
	msleep(500);
	pvr_read_mid_voltage_chan(chg, &pval);
	smblib_err(chg, "dcin_aicl DCIN_P voltage = %d \n", pval.intval);
	if((pval.intval > DCIN_VOL_THRES ) || (pval.intval  < 1000000)){
	        chg->wls_icl_ua = 1100000;
	} else {
		chg->wls_icl_ua = 1100000;
	}
	/* Added by gavin.zhao for task_58666 DC_IN charging debug --end */
