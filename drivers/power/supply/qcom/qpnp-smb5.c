...
/* Added by gavin.zhao for task_58666 DC_IN charging debug --begin */
#include <linux/gpio.h>
#include <linux/slab.h>
#include <linux/of_gpio.h>
/* Added by gavin.zhao for task_58666 DC_IN charging debug --end */
...
struct smb_dt_props {
	...
	bool			no_battery;
	bool			hvdcp_disable;
/* Added by PICO_Driver gavin.zhao for bug31271：force 9v output --start-- */
	bool    qc3_disable;
/* Added by PICO_Driver gavin.zhao for bug31271：force 9v output --end-- */
...
/* Added by gavin.zhao for task_58666 DC_IN charging debug --begin */
static irqreturn_t dc_input_change_interrupt(int irq, void *data)
{
	struct smb_charger *chg = data;

	chg->dc_input_change_status = gpio_get_value(chg->dc_input_change_gpio);
	pr_err("dc input change status = %d\n", chg->dc_input_change_status);
...
/* Added by PICO_Driver gavin.zhao for bug31271：force 9v output --start-- */
	chip->dt.qc3_disable = of_property_read_bool(node,"qcom,qc3-disable");
	printk( KERN_ERR "chip->dt.qc3_disable = %d\n", chip->dt.qc3_disable);
/* Added by PICO_Driver gavin.zhao for bug31271：force 9v output --end-- */
	...
	/* Added by gavin.zhao for task_58666 DC_IN charging debug --begin */
         chg->dc_input_change_gpio = of_get_named_gpio(node, "qcom,dc-input-change", 0);
         if (gpio_is_valid(chg->dc_input_change_gpio)) {
                 gpio_request(chg->dc_input_change_gpio, "qcom,dc-input-change");
                 gpio_direction_input(chg->dc_input_change_gpio);

                 chg->dc_input_change_irq = gpio_to_irq(chg->dc_input_change_gpio);
                 if((rc = request_irq(chg->dc_input_change_irq, dc_input_change_interrupt,
                                 IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "irq", chg)) < 0)
                 {
                        pr_err("dc input chage request  irq error\n");
                 }
         }
	/* Added by gavin.zhao for task_58666 DC_IN charging debug --end */
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	/* Added for bug_63594 fix HUAWEI type_c from Qcom  --begin */
	rc = smblib_masked_write(chg, TYPE_C_CFG_REG, BC1P2_START_ON_CC_BIT, 0);
	if (rc < 0) {
		 dev_err(chg->dev, "failed to write TYPE_C_CFG_REG rc=%d\n",rc);
	}
	/* Added for bug_63594 fix HUAWEI type_c from Qcom  --end */
...
static struct smb_irq_info smb5_irqs[] = {
	...
	[CHGR_ERROR_IRQ] = {
		.name		= "chgr-error",
		.handler	= error_default_irq_handler,
	},
...
/*Modify by PICO_Driver janvan.an for task15516 : add debufs file to disable charge --start--*/
/*
 * to disable charge : echo 1 > /sys/kernel/debug/qpnp-smbcharger/disable_chg
 * to enable charge : echo 0 > /sys/kernel/debug/qpnp-smbcharger/disable_chg
 * default status: charge enabled
 */
/*Modify by PICO_Driver gavin.zhao for bug_32041 : add 500mA charge current --start--*/
static int pico_val = 0;
static int disable_chg_write(void *data, u64 val)
{
    struct smb_charger *chip = data;
	int rc = 0;

    printk( KERN_ERR "psensor charge set = %llu\n", val);
    if((val == 1) || (val == 0)) {
	    vote(chip->fcc_votable, USER_DAEMON_VOTER, false, 0);
	    rc = vote(chip->chg_disable_votable, USER_VOTER, val, 0);
	    if (rc < 0) {
		dev_err(chip->dev, "Couldn't enable charging rc=%d\n", rc);
		return rc;
	    }
    } else if((val == 2) && (pico_val != 2)){
	    rc = vote(chip->chg_disable_votable, USER_VOTER, 0, 0);
	    if (rc < 0) {
		dev_err(chip->dev, "Couldn't enable charging rc=%d\n", rc);
		return rc;
	    }
	    vote(chip->fcc_votable, USER_DAEMON_VOTER, true, 500000);
	    printk( KERN_ERR "psensor charge set 500ma \n");
    } else if((val == 3) && (pico_val != 3)){
	    rc = vote(chip->chg_disable_votable, USER_VOTER, 0, 0);
	    if (rc < 0) {
		dev_err(chip->dev, "Couldn't enable charging rc=%d\n", rc);
		return rc;
	    }
	    vote(chip->fcc_votable, USER_DAEMON_VOTER, true, 800000);
	    printk( KERN_ERR "psensor charge set 800ma \n");
    } else {
	    printk( KERN_ERR "psensor charge set err \n");
    }
    pico_val = val;
    return 0;
}
/*Modify by PICO_Driver gavin.zhao for bug_32041 : add 500mA charge current --end--*/
static int disable_chg_read(void *data, u64 *val)
{
    *val = pico_val;
    return 0;
}
DEFINE_DEBUGFS_ATTRIBUTE(disable_chg_ops,disable_chg_read,disable_chg_write, "0x%02llx\n");

/* Added by gavin.zhao for task_58666 DC_IN charging debug --begin */
static int dc_input_write(void *data, u64 val)
{
   return 0;
}
static int dc_input_read(void *data, u64 *val)
{
    struct smb_charger *chg = data;

    *val = chg->dc_input_change_status;
    return 0;
}
DEFINE_DEBUGFS_ATTRIBUTE(dc_input_ops,dc_input_read,dc_input_write, "0x%02llx\n");
/* Added by gavin.zhao for task_58666 DC_IN charging debug --end */
/*Modify by PICO_Driver janvan.an for task15516 : add debufs file to disable charge --end--*/
static void smb5_create_debugfs(struct smb5 *chip)
{
	struct dentry *file;

/*Modify by PICO_Driver janvan.an for task15516 : add debufs file to disable charge --start--*/
	chip->dfs_root = debugfs_create_dir("qpnp-smbcharger", NULL);
/*Modify by PICO_Driver janvan.an for task15516 : add debufs file to disable charge --end--*/
...
/*Modify by PICO_Driver janvan.an for task15516 : add debufs file to disable charge --start--*/
	file = debugfs_create_file("disable_chg",
				S_IFREG | S_IWUSR | S_IRUGO | S_IWUGO,
				chip->dfs_root, chip,
				&disable_chg_ops);
	if (IS_ERR_OR_NULL(file))
		pr_err("Couldn't create disable chg file rc=%ld\n",
			(long)file);
/*Modify by PICO_Driver janvan.an for task15516 : add debufs file to disable charge --end--*/
/* Added by gavin.zhao for task_58666 DC_IN charging debug --begin */
	file = debugfs_create_file("dc_input",
				S_IFREG | S_IWUSR | S_IRUGO | S_IWUGO,
				chip->dfs_root, chip,
				&dc_input_ops);
	if (IS_ERR_OR_NULL(file))
		pr_err("Couldn't create dc_input file rc=%ld\n",
			(long)file);
/* Added by gavin.zhao for task_58666 DC_IN charging debug --end */
}
