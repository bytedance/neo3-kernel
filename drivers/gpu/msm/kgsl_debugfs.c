...
char *get_dma_buf_name(struct kgsl_mem_entry *entry);
static int print_mem_entry(void *data, void *ptr)
{
	...
	if (entry->priv_data)
		name = get_dma_buf_name(entry);

	seq_printf(s, "%pK %pK %16llu %5d %9s %10s %16s %5d %16d %6d %6d %6d %6llu %6llu %6llu %10s",
			...
			egl_surface_count, egl_image_count, total_count,
            atomic_long_read(&m->physsize), entry_mapped, entry_unmapped,
            name);
...
{
	if (ptr == SEQ_START_TOKEN) {
		seq_printf(s, "%16s %16s %16s %5s %9s %10s %16s %5s %16s %6s %6s %6s %6s %6s %6s %6s\n",
			"gpuaddr", "useraddr", "size", "id", "flags", "type",
			"usage", "sglen", "mapcount", "eglsrf", "eglimg", "totalcount",
            "physsize", "mapped", "unmapped", "dmaname");
		return 0;
	} else
