...
#define NXP_FEM_ELF_BDF_WITH_FULL_PATH   "qca6390/bdwlann.elf"
#define QORVO_FEM_ELF_BDF_WITH_FULL_PATH "qca6390/bdwlanq.elf"
#define DEFAULT_ELF_BDF_WITH_FULL_PATH   "qca6390/bdwlan.elf"
...
#define PHOENIX_FEM_IS_NXP 0
#define PHOEMIX_FEM_IS_QORVO 1

extern int cnss_wlan_rffemid;
				  ...
				  u32 filename_len)
{
	...
	case CNSS_BDF_ELF:
		...
		if (plat_priv->board_info.board_id == 0xFF) {
			if (cnss_wlan_rffemid == PHOENIX_FEM_IS_NXP) {
				snprintf(filename_tmp, filename_len,
					BDF_FILE_NAME_PREFIX "n.elf");
			} else if (cnss_wlan_rffemid == PHOEMIX_FEM_IS_QORVO) {
				snprintf(filename_tmp, filename_len,
					BDF_FILE_NAME_PREFIX "q.elf");
			} else {
				snprintf(filename_tmp, filename_len, ELF_BDF_FILE_NAME);
			}
			// cnss_pr_err("Enter %s: %d file=%s",__func__,__LINE__,filename_tmp);
				 ...
				 u32 bdf_type)
{
	...
	if (ret) {
		if ((strcmp(filename, QORVO_FEM_ELF_BDF_WITH_FULL_PATH) == 0 ) ||
			(strcmp(filename, NXP_FEM_ELF_BDF_WITH_FULL_PATH) == 0 )) {
			cnss_pr_err("Failed to load BDF %s => try to load default BDF: %s\n", filename, DEFAULT_ELF_BDF_WITH_FULL_PATH);
			strcpy(filename, DEFAULT_ELF_BDF_WITH_FULL_PATH);
			ret = request_firmware(&fw_entry, filename, &plat_priv->plat_dev->dev);
			if (ret) {
