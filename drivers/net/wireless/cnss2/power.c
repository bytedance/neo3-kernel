...
#include "debug.h"
#include "bus.h"
/*Modify by PICO_Driver clark.tian for bug62692: Auto set vdd-wlan-dig --start--*/
#include <linux/hardware_version.h>

static struct cnss_vreg_cfg cnss_vreg_list_pm8009[] = {
...
static struct cnss_vreg_cfg cnss_vreg_list_pm8150[] = {
	...
	{"wlan-soc-swreg", 1200000, 1200000, 0, 0, 0},
	{"vdd-wlan-aon", 950000, 950000, 0, 0, 0},
	{"vdd-wlan-dig-pm8150", 950000, 952000, 0, 0, 0},
	{"vdd-wlan-rfa1", 1900000, 1900000, 0, 0, 0},
	{"vdd-wlan-rfa2", 1350000, 1350000, 0, 0, 0},
	{"vdd-wlan-en", 0, 0, 0, 10, 0},
};
/*Modify by PICO_Driver clark.tian for bug62692: Auto set vdd-wlan-dig --end--*/
static struct cnss_clk_cfg cnss_clk_list[] = {
	{"rf_clk", 0, 0},
};
/*Modify by PICO_Driver clark.tian for bug62692: Auto set vdd-wlan-dig --start--*/
#define CNSS_VREG_INFO_SIZE		ARRAY_SIZE(cnss_vreg_list_pm8009)
#define USE_PM8009				0
#define USE_PM8150				1
/*Modify by PICO_Driver clark.tian for bug62692: Auto set vdd-wlan-dig --end--*/
...
#define WLAN_RFFEMID_GPIO		"wlan-rffemid-gpio"
...
/*Add by PICO_Driver clark.tian for bug62692: Auto set vdd-wlan-dig --start--*/
/*
*	Hardware version B3 use pm8009,return 0;
*	Hardware version B4/B5 use pm8150,return 1;
*	Default use pm8009,return 0
*/
static int check_pm_config(void)
{
	int version = 0;
	version = hw_version_get();
	#if PM8009_CONFIG
	switch (version){
        case 0:
        case 1:
					   ...
					   enum cnss_vreg_type type)
{
	...
	case CNSS_VREG_PRIM:
		*vreg_list_size = CNSS_VREG_INFO_SIZE;
		/*Modify by PICO_Driver clark.tian for bug62692: Auto set vdd-wlan-dig --start--*/
		if(check_pm_config()){
			return cnss_vreg_list_pm8150;
		}else{
			return cnss_vreg_list_pm8009;
		}
		/*Modify by PICO_Driver clark.tian for bug62692: Auto set vdd-wlan-dig --end--*/
	default:
...
int cnss_get_pinctrl(struct cnss_plat_data *plat_priv)
{
	...
	/*get wlan fem_id information */
	if (of_find_property(dev->of_node, WLAN_RFFEMID_GPIO, NULL)) {
		pinctrl_info->wlan_rffemid_gpio = of_get_named_gpio(dev->of_node,
							    WLAN_RFFEMID_GPIO, 0);
		cnss_pr_dbg("WLAN RF FEMID GPIO: %d\n", pinctrl_info->wlan_rffemid_gpio);
	} else {
		pinctrl_info->wlan_rffemid_gpio = -EINVAL;
	}
...
static int cnss_select_pinctrl_enable(struct cnss_plat_data *plat_priv)
{
	...
	cnss_pr_dbg("Enter function %s:%d \n",__func__,__LINE__);
...
set_wlan_en:
	...
	cnss_pr_dbg("Enter function %s:%d \n",__func__,__LINE__);
	gpio_direction_output(plat_priv->pinctrl_info.wlan_rffemid_gpio, 0);
	gpio_direction_input(plat_priv->pinctrl_info.wlan_rffemid_gpio);
	msleep(10);
	if(gpio_get_value(plat_priv->pinctrl_info.wlan_rffemid_gpio)) {
		cnss_wlan_rffemid = 1;
		cnss_pr_dbg("RF FEMID is : 1\n");
	} else {
		cnss_wlan_rffemid = 0;
		cnss_pr_dbg("RF FEMID is : 0\n");
	}
	cnss_pr_err("Enter %s: %d\n cnss_wlan_rffemid=%d ",
		     __func__,__LINE__,cnss_wlan_rffemid);
