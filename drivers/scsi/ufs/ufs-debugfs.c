...
static int ufsdbg_dump_device_desc_show(struct seq_file *file, void *data)
{
	...
	struct ufs_hba *hba = (struct ufs_hba *)file->private;
      /*modify by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --start--*/
      ...
      /*modify by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --end--*/
	pm_runtime_get_sync(hba->dev);
...
/*added by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --start--*/
static int ufsdbg_unit_str_show(struct seq_file *file, void *data)
{
    struct ufs_hba *hba = (struct ufs_hba *)file->private;
    //struct ufs_stats *ufs_stats = &hba->ufs_stats;

    u8 dev_unit_desc_buf[QUERY_DESC_MAX_SIZE]={0};

    int i,err;

    struct desc_field_offset dev_unit_desc_field_name[] = {
        {"bLength",		0x00, BYTE},
        {"bDescriptorIDN",	0x01, BYTE},
        ...
        {"qLogicalBlockCount",		0x0B, BYTE_8},
        {"dEraseBlockSize",		0x13, DWORD},
        {"bProvisioningType",		0x17, BYTE},
        {"qPhyMemResourceCount",		0x18, BYTE_8},
        {"wContextCapabilities",		0x20, WORD},
        {"bLargeUnitGranularity_M1",		0x22, BYTE},
    };

    seq_printf(file,"\tdevice unit_desc descriptor lenght =%d\n",hba->desc_size.unit_desc);
    //printk(KERN_ERR"%s line%d i_manufacturer_name=%d\n",__func__,__LINE__,hba->dev_info.i_manufacturer_name);

    pm_runtime_get_sync(hba->dev);
    err = ufshcd_read_unit_desc(hba,dev_unit_desc_buf,hba->desc_size.unit_desc);
    pm_runtime_put_sync(hba->dev);
    if (!err) {
        struct desc_field_offset *tmp;
        for (i = 0; i < ARRAY_SIZE(dev_unit_desc_field_name); ++i) {
            tmp = &dev_unit_desc_field_name[i];

            if (tmp->width_byte == BYTE) {
                seq_printf(file,
                    "Device Unit String Descriptor[Byte offset 0x%x]: %s = 0x%x\n",
                    tmp->offset,
                    tmp->name,
                    dev_unit_desc_buf[tmp->offset]);
            } else if (tmp->width_byte == WORD) {
                seq_printf(file,
                    "Device Unit String Descriptor[Byte offset 0x%x]: %s = %x-%c\n",
                    tmp->offset,
                    tmp->name,
                    dev_unit_desc_buf[tmp->offset],
                    dev_unit_desc_buf[tmp->offset]);
                ...
                seq_printf(file,
                    ...
                    tmp->offset,
                    tmp->name,
                    *(u32 *)&dev_unit_desc_buf[tmp->offset]);
            } else if (tmp->width_byte == BYTE_8) {
                seq_printf(file,
                    "Device Unit String Descriptor[Byte offset 0x%x]: %s = 0x%x\n",
                    tmp->offset,
                    tmp->name,
                ...
                seq_printf(file,
                    "Device Unit String Descriptor[offset 0x%x]: %s. Wrong Width = %d\n",
                    tmp->offset, tmp->name, tmp->width_byte);
            }
        }
    }else {
        seq_printf(file, "Device Unit String Descriptor Descriptor failed. err = %d\n",err);
    }
    return err;

}

static int ufsdbg_unit_descriptor_open(struct inode *inode, struct file *file)
{
	return single_open(file, ufsdbg_unit_str_show, inode->i_private);
}

static const struct file_operations ufsdbg_unit_descriptor_fops = {
	.open		= ufsdbg_unit_descriptor_open,
	.read		= seq_read,
	//.write		= ufsdbg_query_stats_write,
	.release	= single_release,
};
/*added by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --end--*/
...
void ufsdbg_add_debugfs(struct ufs_hba *hba)
{
	...
	hba->debugfs_files.dump_dev_desc =
		debugfs_create_file("dump_device_desc", 0600,
				    hba->debugfs_files.debugfs_root, hba,
	...
	/*added by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --start--*/
	hba->debugfs_files.unit_descr =
		debugfs_create_file("dump_unit_desc", S_IRUSR | S_IWUSR,
					   hba->debugfs_files.debugfs_root, hba,
					   &ufsdbg_unit_descriptor_fops);
	if (!hba->debugfs_files.unit_descr) {
		dev_err(hba->dev, "%s:  NULL manufacturer_descr file, exiting",
			__func__);
		goto err;
	}
	/*added by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --end--*/
