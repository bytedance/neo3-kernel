...
int ufshcd_read_unit_desc(struct ufs_hba *hba,
					 u8 *buf,
					 u32 size)
{
	return ufshcd_read_desc(hba, QUERY_DESC_IDN_UNIT, 0, buf, size);
}
...
static int ufs_read_device_desc_data(struct ufs_hba *hba)
{
	...
	/*added by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --start--*/
	hba->dev_info.i_manufacturer_name = desc_buf[DEVICE_DESC_PARAM_MANF_NAME];
	hba->dev_info.i_oem_id = desc_buf[DEVICE_DESC_PARAM_OEM_ID];
	printk(KERN_ERR"%s ufs line%d i_manufacturer_name=%d,i_oem_id=%d\n",__func__,__LINE__,hba->dev_info.i_manufacturer_name,hba->dev_info.i_oem_id);
	/*added by PICO_Driver haiyun.zhao for Bug 68373: read info of ufs   --end--*/
