...
/*Added by PICO_Driver jim.qiu for bug56759 : debug for usb3.0 phy start */
...
/*Added by PICO_Driver jim.qiu for bug56759 : debug for usb3.0 phy end */
...
/*Added by PICO_Driver jim.qiu for bug56759 : debug for usb3.0 phy start */
static struct dentry *debugfs_dp_txmgn_v0;
/*Added by PICO_Driver jim.qiu for bug56759 : debug for usb3.0 phy end */
...
/*Added by PICO_Driver jim.qiu for bug56759 : debug for usb3.0 phy start */
static int dp_txmgn_v0_debug_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}

static ssize_t dp_txmgn_v0_debug_write(struct file *filep,
	const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	struct msm_ssphy_qmp *priv_data = filep->private_data;

	if (*ppos)
		return 0;

	printk("dp_txmgn_v0_debug_write cnt is %d\n",cnt);

	wlen = min_t(ssize_t, cnt, SZ_16-1);

	printk("dp_txmgn_v0_debug_write wlen is %d\n",wlen);

	rc = copy_from_user(wbuf, ubuf, wlen);
	if (rc)
		return -EFAULT;

	wbuf[wlen] = '\0';

	if (kstrtou8(wbuf, 0, &val))
		return -EINVAL;

	printk("dp_txmgn_v0_debug_write val is 0x%x\n",val);

        writeb_relaxed(val,priv_data->base + USB3_DP_PCS_G12S1_TXMGN_V0);

	return wlen;
}

static ssize_t dp_txmgn_v0_debug_read(struct file *file, char __user *ubuf,
				size_t count, loff_t *ppos)
{
	...
	char tmp_buf[SZ_16]={0};

	struct msm_ssphy_qmp *priv_data = file->private_data;

	if (*ppos)
		return 0;

	reg_val = readb_relaxed(priv_data->base + USB3_DP_PCS_G12S1_TXMGN_V0);

	printk("dp_txmgn_v0_debug_read is 0x%x\n",reg_val);

	ret_cnt += snprintf(tmp_buf, SZ_16,"0x%x\n", reg_val);

	printk("dp_txmgn_v0_debug_read ret_cnt is %d,count=%d\n",ret_cnt,count);

	ret_cnt = min_t(ssize_t, count, ret_cnt);
	if (copy_to_user(ubuf, tmp_buf, ret_cnt)) {
		pr_err("%s: fail to copy_to_user\n", __func__);
		return  -EFAULT;
	}
...
static const struct file_operations dp_txmgn_v0_debug_ops = {
	.open = dp_txmgn_v0_debug_open,
	.write = dp_txmgn_v0_debug_write,
	.read = dp_txmgn_v0_debug_read,
};
...
static void msm_ssphy_create_debugfs(struct msm_ssphy_qmp *phy)
{
	...
	if (!IS_ERR(phy->root)) {
				debugfs_dp_txmgn_v0 = debugfs_create_file("dp_txmgn_v0",
						0666, phy->root,
						(void *) phy,
						&dp_txmgn_v0_debug_ops);
	}

	#if 0
	debugfs_create_x8("dp_txmgn_v0", 0666, phy->root, phy->base+USB3_DP_PCS_G12S1_TXMGN_V0);
	debugfs_create_x8("dp_m3p5_db", 0666, phy->root, phy->base+USB3_DP_PCS_G12S1_TXDEEMPH_M3P5DB);
	debugfs_create_x8("uni_txmgn_v0", 0666, phy->root, phy->base+USB3_UNI_PCS_G12S1_TXMGN_V0);
	debugfs_create_x8("uni_m3p5_db", 0666, phy->root, phy->base+USB3_UNI_PCS_G12S1_TXDEEMPH_M3P5DB);
	debugfs_create_x8("dp_txa", 0666, phy->root, phy->base+USB3_DP_QSERDES_TXA_TX_DRV_LVL);
	debugfs_create_x8("dp_txb", 0666, phy->root, phy->base+USB3_DP_QSERDES_TXB_TX_DRV_LVL);
	debugfs_create_x8("uni_tx", 0666, phy->root, phy->base+USB3_UNI_QSERDES_TX_TX_DRV_LVL);
	#endif
}
/*Added by PICO_Driver jim.qiu for bug56759 : debug for usb3.0 phy end */
...
static int msm_ssphy_qmp_probe(struct platform_device *pdev)
{
	...
	/*Added by PICO_Driver jim.qiu for bug56759 : debug for usb3.0 phy start */
	if(USB_PHY_TYPE_USB3_AND_DP == phy->phy.type)
