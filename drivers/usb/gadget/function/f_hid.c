...
static struct usb_function *hidg_alloc(struct usb_function_instance *fi)
{
	...
	if (opts->report_desc) {
		...
		if (!hidg->report_desc) {
            ...
            hidg->report_length = 64;
            hidg->report_desc_length = 29;

            hidg->report_desc = kzalloc(sizeof(report_desc_hid), GFP_KERNEL);
            memcpy(hidg->report_desc, report_desc_hid, sizeof(report_desc_hid));
	}
