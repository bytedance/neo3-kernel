...
static bool msm_swap_gnd_mic(struct snd_soc_component *component, bool active)
{
	...
	dev_dbg(card->dev, "%s called by %pf\n",__func__,__builtin_return_address(0));
...
static void *def_wcd_mbhc_cal(void)
{
	...
	btn_high[0] = 75;
	btn_high[1] = 148;//origin:150
	btn_high[2] = 223;//origin:237
	btn_high[3] = 500;
	btn_high[4] = 500;
	btn_high[5] = 500;
	btn_high[6] = 500;
...
static struct snd_soc_dai_link msm_common_misc_fe_dai_links[] = {
	...
	{/* hw:x,39 */
     ...
     /*Added by PICO_Driver jim.qiu for task58976 :mi2s0 loop back to speaker --start--*/
	 {/* hw:x,40 */
	 ...
	 {/* hw:x,42 */
		.name = "WSA_DMA_RX_0_HOSTLESS",
		.stream_name = "WSA_DMA_RX_0_HOSTLESS Playback",
		.cpu_dai_name = "WSA_DMA_RX_0_HOSTLESS",
		.platform_name = "msm-pcm-hostless",
		.dynamic = 1,
		.dpcm_playback = 1,
		.trigger = {SND_SOC_DPCM_TRIGGER_POST,
	...
	/*Added by PICO_Driver jim.qiu for task58976 :mi2s0 loop back to speaker --end--*/
};
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	//Audio: swap_mic_gnd compatible with ti and qcom {@
	kona_hw_version = get_hw_version();
	dev_info(&pdev->dev, "%s: <Audio> hw_version = %d\n",
		__func__, kona_hw_version);
	//Audio: @}
	...
	if (!pdata->us_euro_gpio_p) {
		...
		dev_dbg(&pdev->dev, "%s detected\n",
			"qcom,us-euro-gpios");
		//Audio: swap_mic_gnd compatible with ti and qcom {@
		if (kona_hw_version < NEO3_NEW_JACK_HW)
			wcd_mbhc_cfg.swap_gnd_mic = NULL;
		else
