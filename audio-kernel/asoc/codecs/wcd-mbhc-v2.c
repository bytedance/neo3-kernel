...
/*Added by PICO_Driver jim.qiu for bug54370 : bring up audio,for debugging the TS3A226AEYFFR which is compatible switch of headphone --start--*/
#include <linux/clk.h>
...
//Audio: swap_mic_gnd compatible with ti and qcom {@
#include <linux/hardware_version.h>
//Audio: @}

/* PICO add mbhc fs debug interface */
...
int ti_swap_gnd_mic(struct snd_soc_card *card, bool active)
{
	int ret = -1;
	//struct snd_soc_card *card = codec->component.card;
	struct device_node *us_euro_gpio_p = NULL;

	dev_dbg(card->dev, "%s called by %pf,active=%d\n",__func__,__builtin_return_address(0),active);

	us_euro_gpio_p = of_parse_phandle(card->dev->of_node,
					"qcom,us-euro-gpios", 0);

	if (!us_euro_gpio_p) {
		dev_err(card->dev, "%s: property %s not detected in node %s",
			"qcom,us-euro-gpios", card->dev->of_node->full_name);
		return -EINVAL;
	}

	if (active) {
		ret = msm_cdc_pinctrl_select_active_state(us_euro_gpio_p);
		if (ret) {
			dev_err(card->dev, "%s: msm_cdc_pinctrl_select_active_state fail! ret=%d\n",__func__,ret);
		}
	} else {
		ret = msm_cdc_pinctrl_select_sleep_state(us_euro_gpio_p);
		if (ret) {
			dev_err(card->dev, "%s: msm_cdc_pinctrl_select_sleep_state fail! ret=%d\n",__func__,ret);
		}
	}
	dev_dbg(card->dev, "%s end,ret=%d\n",__func__,ret);

	return ret;
}
EXPORT_SYMBOL(ti_swap_gnd_mic);
/*Added by PICO_Driver jim.qiu for bug54370 : bring up audio,for debugging the TS3A226AEYFFR which is compatible switch of headphone --end--*/

//Audio: swap_mic_gnd compatible with ti and qcom {@
int g_kona_hw_version = -1;
int get_hw_version(void)
{
    g_kona_hw_version = hw_version_get();
    return g_kona_hw_version;
}
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	//Audio: swap_mic_gnd compatible with ti and qcom {@
	/* get neo3 hw version */
	if (g_kona_hw_version == -1)
		get_hw_version();
	dev_dbg(component->dev, "%s: <Audio> hw_version = %d\n", __func__, g_kona_hw_version);
	//Audio: @}
	/* cancel pending button press */
	if (wcd_cancel_btn_work(mbhc))
		pr_debug("%s: button press is canceled\n", __func__);
	...
	if ((mbhc->current_plug == MBHC_PLUG_TYPE_NONE) &&
	    detection_type) {
		//Audio: swap_mic_gnd compatible with ti and qcom {@
		if (g_kona_hw_version < NEO3_NEW_JACK_HW)
			ti_swap_gnd_mic(component->card,true);
		//Audio: @}
		...
		//Audio: swap_mic_gnd compatible with ti and qcom {@
		if (g_kona_hw_version < NEO3_NEW_JACK_HW)
			ti_swap_gnd_mic(component->card,false);
		//Audio: @}
											...
											size_t count, loff_t *pos)
{
	...
	if (!buffer) {
		dev_err(component->dev, "alloc buffer failed\n");
		return -ENOMEM;
	}
												...
												size_t count, loff_t *pos)
{
	...
	/* Coverity CWE-119: Fix out of bounds writes buffer */
	kbuf = kzalloc(count + 1, GFP_KERNEL);
	if (!kbuf) {
		dev_err(component->dev, "alloc kbuf failed.\n");
		return -ENOMEM;
	}
	...
	if (rc < 0) {
		dev_err(component->dev, "input error.\n");
		return -EINVAL;
	}
	...
	dev_info(component->dev, "Got from user:\n");
	for (i = 0; i < 8; i++) {
		dev_info(component->dev, "btn[%d] = %d\n", i, btn[i]);
		      ...
		      bool impedance_det_en)
{
	...
	// Pico-Audio Start: AUDIO_B00010: fix bug5032779
	if (g_kona_hw_version == -1)
		get_hw_version();
	if (g_kona_hw_version >= 0 && g_kona_hw_version < NEO3_NEW_JACK_HW)
		ti_swap_gnd_mic(card,false);
	// Pico-Audio End: AUDIO_B00010
